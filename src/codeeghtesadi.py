...
if __name__ == '__main__':
    # index = 'final_haghighi_all'
    # index = 'final_hoghoghi_all'
    # file_path = r'E:\automating_reports_V2\saved_dir\codeghtesadi\imp_corps_1401\%s.xlsx' % index
    # file_path_haghighi = r'E:\automating_reports_V2\saved_dir\codeghtesadi\imp_corps_1401\backup\final_haghighi_all.xlsx'
    # file_path_hoghoghi = r'E:\automating_reports_V2\saved_dir\codeghtesadi\imp_corps_1401\backup\final_hoghoghi_all.xlsx'

    # df = pd.read_excel(file_path_hoghoghi, sheet_name='Sheet1')

    # df = np.array_split(df, 8)

    # for index, item in enumerate(df):
    #     path = r'E:\automating_reports_V2\saved_dir\codeghtesadi\imp_corps_1401\part%s.xlsx' % index
    #     item.to_excel(path, index=False)

    # print('f')

    # df.sort_values(by=['شناسه ملی'], inplace=True)
    # df.drop_duplicates(subset=['شناسه ملی'], inplace=True)
    # count = count_num_files(
    #     r'E:\automating_reports_V2\saved_dir\test\1400-1401\%s' % index)
    # count *= 10

    # df = df.iloc[count:, :]
    print('Starting the schedule...')
    # path = r'E:\automating_reports_V2\saved_dir\test\1400-1401'
    # path = r'E:\automating_reports_V2\saved_dir\codeghtesadi\imp_corps_1401'
